...
Texture::Texture(const char *filename)
{
	...
	if (error)
	{
		std::cout << "decoder error " << error << ": " << lodepng_error_text(error) << std::endl << std::endl; return;
	}
	loaded = true;
...
btVector3 Texture::get_pixel(unsigned x, unsigned y) const
{
...
btVector3 Texture::get_pixel(double u, double v) const
{
